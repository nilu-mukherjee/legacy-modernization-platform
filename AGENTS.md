...
- **AI:** Anthropic Codex SDK + OpenAI SDK (provider-agnostic)
...
1. **Two-tier AI strategy:** 80% deterministic analysis (tree-sitter + rules = free),
   20% LLM-powered semantic analysis (Codex/OpenAI = paid). This keeps costs to
...
6. AI Analysis (70-90%)  → Codex Haiku (bulk) + Sonnet (synthesis)
...
| `backend/app/services/ai_pipeline.py` | Codex/OpenAI integration |
